...
# ==========update tree View
updateTV = ttk.Treeview(height=15, columns=(
    'Name', 'Rate', 'type', 'Store_Type'))
...
def generate_bill():
    ...
    conn.commit()
    conn.close()
    listDict = {"name": itemName, "rate": itemRate,
...
def updateListView():
    ...
    for row in itemLists:
        billsTV.insert('', 'end', text=row['name'], values=(
            row["rate"], row["quantity"], row["cost"]))
...
def getItemLists():
    ...
    for row in data:
        updateTV.insert('', 'end', text=row['nameid'], values=(
            row['name'], row['rate'], row['type'], row['storetype']))
...
def loginWindow():
    titleLabel = Label(window, text="Billing System",
                       fg="green", font="Times 40 bold")
    titleLabel.grid(row=0, column=0, columnspan=4,
                    padx=(40, 280), pady=(10, 100))

    loginLabel = Label(window, text="Admin Login", font="Times 20 bold")
    loginLabel.grid(row=1, column=2, padx=(100, 100),
                    pady=(10, 20), columnspan=2)

    usernameLabel = Label(window, text="Username", font="Times 15 bold", bd=10)
    usernameLabel.grid(row=2, column=2, padx=20, pady=5)

    passwordLabel = Label(window, text="Password", font="Times 15 bold", bd=10)
    ...
    loginButton = Button(window, text="Login", width=20,
                         height=2, command=lambda: adminLogin(), font="Times 13 bold", bd=5)
    loginButton.grid(row=4, column=2, columnspan=2, pady=10)


def mainwindow():

    titleLabel = Label(window, text="Billing System",
                       font="Times 30 bold", fg="green")
    titleLabel.grid(row=0, column=1, columnspan=3, pady=(10, 0), padx=10)

    addNewItem = Button(window, text="Add Item", width=15,
                        height=2, command=lambda: addItemListener())
    addNewItem.grid(row=1, column=0, padx=(10, 0), pady=(10, 0))
    updateItem = Button(window, text="Update Item", width=15,
                        height=2, command=lambda: moveToUpdate())
    updateItem.grid(row=1, column=1, padx=(10, 0), pady=(10, 0))

    showallEntry = Button(window, text="Show Bills",
                          width=15, height=2, command=lambda: movetoBills())
    showallEntry.grid(row=1, column=2, padx=(10, 0), pady=(10, 0))

    logoutBtn = Button(window, text="Logout", width=15,
                       height=2, command=lambda: LogOut())
    ...
    itemDropDown = OptionMenu(window, itemVariable,
                              *options, command=optionMenuListener)
    ...
    buttonBill = Button(window, text="Add to List", width=15,
                        command=lambda: generate_bill())
    buttonBill.grid(row=3, column=4, padx=(5, 0), pady=(10, 0))

    billLabel = Label(window, text="Bills", font="Times 25 bold")
    ...
    generateBill = Button(window, text="Generate Bills",
                          width=15, command=lambda: print_bill())
...
def itemAddWindow():
    ...
    titleLabel = Label(window, text="Billing System",
                       width=40, font="Times 30 bold", fg="green")
    titleLabel.grid(row=0, column=2, columnspan=4)
    ...
    AddItemButton = Button(window, text="Add Item",
                           width=20, height=2, command=lambda: addItem())
...
def updateItemWindow():
    ...
    AddItemButton = Button(window, text="Update Item",
                           width=20, height=2, command=lambda: updateItem())
    AddItemButton.grid(row=3, column=3, pady=(10, 0))
